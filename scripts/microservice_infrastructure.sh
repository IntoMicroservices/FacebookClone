...
#  launchBrowser http://localhost:10020/dummya-service-app/default config-server &
  ...
  launchBrowser http://localhost:10040/actuator/gateway/routes api-gateway &
  feedRedis redis &
elif [ "$1" == "down" ]; then
...
#images are pushed to local registry
  (cd "$SCRIPTS_DIR"/.. || exit; mvn -pl config-server,service-registry,api-gateway -am install -P docker-images || exit)
else
